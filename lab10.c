...
1. How is the client sending data to the server? What protocol?
  TCP over IP, connecting to localhost:8001 using SOCK_STREAM.

2. What data is the client sending to the server?
  String messages: "Hello", "Apple", "Car", "Green", "Dog": 5 per client.


Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  A struct acceptor_args that has: a run flag, a pointer to the
  shared linked list, and a pointer to the list mutex.

2. How are received messages stored?
  A linked list. Each message is a malloc'd list_node appended via add_to_list(), protected
  by a mutex.

3. What does `main()` do with the received messages?
  Busy-waits until 20 messages are received, then stops the acceptor thread and calls
  collect_all() to print and free every node in the list.

4. How are threads used in this sample code?
  One acceptor thread listens for connections. One client thread per connected client reads
  messages and adds them to the shared list.


Non-blocking sockets:
1. How are sockets made non-blocking?
  Using fcntl() with F_GETFL then F_SETFL with O_NONBLOCK added.

2. What sockets are made non-blocking?
  The server socket (sfd) and each client socket (cfd).

3. Why? What purpose does it serve?
  So accept() and read() return immediately instead of blocking forever.
  This lets threads check their run flag and shut down cleanly when signaled.
...
static void *run_acceptor(void *args) {
  ...
  for (int i = 0; i < num_clients; i++) {
    ...
    client_args[i].run = false;     // signal first
    pthread_join(threads[i], NULL); // wait for it
    close(client_args[i].cfd);      // close the sockets
  }
