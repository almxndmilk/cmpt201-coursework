...
1. What is the address of the server it is trying to connect to (IP address and port number).
  ...
  IP: 127.0.0.1 (localhost), Port: 8000


2. Is it UDP or TCP? How do you know?

  TCP. The socket() call uses SOCK_STREAM (not SOCK_DGRAM (UDP)),
  and it calls connect() to establish a connection before sending data.


3. The client is going to send some data to the server. Where does it get this data from? How can
you tell in the code?
  It reads from STDIN. The while loop calls read, meaning it waits for the user to type
  lines and sends each one to the server using write.


4. How does the client program end? How can you tell that in the code?
  The client ends when the user sends an empty line. The while loop condition
  is num_read > 1, so it stops when read() returns 0. It then closes the socket .
